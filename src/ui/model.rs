...
pub fn registry(device_id: &str) -> Vec<Feature> {
    ...
    let mut features = vec![
        ...
        s(
            "steam_downloads",
            "Steam Downloads",
            "On/off updating flag plus a live download % (0-100).",
            Games,
            true,
            Running,
            "76%",
            0,
            "sensor.dank0i_pc_steam_updating + _steam_download",
            "For the live %: an empty '.cef-enable-remote-debugging' file in your Steam folder, then restart Steam",
            "steam_updating (on/off) comes from Steam's .acf files. steam_download (%, speed, app id) attaches to Steam's CEF debugger (127.0.0.1:8080) - pc-bridge drops the enable file automatically; you just restart Steam once.",
        ),
        a(
            "launch_game",
            "Launch Game",
