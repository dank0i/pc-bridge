...
Big release. Full Linux support, a native settings window, live sensor control, and a large round of reliability and security fixes. Changes are grouped below; everything since v2.4.0.

## New features

### Linux is now a first-class target
No external helper tools required anymore. The agent talks to the desktop directly:
...
- **GNOME / KDE Wayland**: idle time via D-Bus (Mutter / KDE ScreenSaver `GetIdletime`), no tools needed.
...
- Linux sensor accuracy pass: disk uses available (non-root) space, CPU includes iowait/irq/softirq/steal, battery reports AC-connected to match Windows.
...
- Fixed an MQTT event-loop deadlock that could occur with many custom topics (reconnect resubscribe now runs off the poll loop).
- Home Assistant discovery is re-published on reconnect, so a broker restart can't orphan entities; entity teardown now also clears retained state/attributes and unsubscribes removed command topics.
...
- OS threads and child processes are now reaped on runtime disable, not just on exit: Linux `gdbus` and `xprop` children are killed and waited (no zombies), Windows event hooks are unhooked.
...
New dependencies added this release (all pure-Rust or already in the tree; the headless agent does not initialize the UI ones):
